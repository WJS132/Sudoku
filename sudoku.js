...
function generowanie() {
    if (fillBoard()) {
        ...
        usuwanie(20); // Usuwamy 40 liczb (możesz dostosować liczbę)
    } else {
...
function fillBoard() {
    for (let row = 0; row < 9; row++) {
        for (let col = 0; col < 9; col++) {
            if (tablica[row][col].textContent === "") { // Szukamy pustego pola
                let liczby = [1, 2, 3, 4, 5, 6, 7, 8, 9];
...
function czyPasuje(row, col, losowa) {
    // Sprawdzamy wiersz
    for (let i = 0; i < 9; i++) {
        if (losowa == parseInt(tablica[row][i].textContent) ) return false;
    }

    // Sprawdzamy kolumnę
    for (let i = 0; i < 9; i++) {
        if (losowa == parseInt(tablica[i][col].textContent)) return false;
    }
    ...
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            if (tablica[startRow + i][startCol + j].textContent == losowa) return false;
        }
    }
...
function wybieraniePola(event){
    const czerwoneDivy = Array.from(document.querySelectorAll(".board div")).filter(div => {
        return getComputedStyle(div).backgroundColor === "rgb(255, 0, 0)";
      });
...
function wpisywanieCyfr(event){
    ...
    console.log(wybranePole.dataset.row, wybranePole.dataset.col, event.target.value);

    if(czyPasuje(parseInt(wybranePole.dataset.row), parseInt(wybranePole.dataset.col), parseInt(event.target.value))){
        console.log("dziwki");
...
for (let row of tablica) {
    for (let pole of row) {
        pole.addEventListener("click", wybieraniePola);
    }
}
